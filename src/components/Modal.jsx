...
function Modal({ setOpenModal, openModal, favorite, setFavorite }) {
  ...
  return (
    <div className="flex items-center justify-center">
      ...
      ></div>
      <div className="bg-gray-800 shadow-sm shadow-slate-400 rounded-xl min-w-96 absolute top-1/3 lg:left-1/3 p-4 text-white my-0 mx-auto">
        ...
        {favorite.length ? (
          <div>
            {favorite.map((item) => {
              return (
                <div
                  key={item.id}
                  className="flex items-center justify-between p-2 bg-gray-700 hover:bg-gray-600  rounded-xl my-3"
                >
                  <img
                  ...
                  <p>{item.name}</p>
                  <TrashIcon
                    className="stroke-red-400 w-6 h-6 ml-20 md:ml-40"
          ...
          <p className="flex items-center justify-center m-3 text-gray-400">
            Your List Is Empty!
          </p>
        )}
      </div>
